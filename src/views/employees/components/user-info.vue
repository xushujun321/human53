...
export default {
  ...
  created() {
    this.getPersonalDetail()// 获取基础信息
    this.getUserDetailById()// 获取个人信息
  },
  methods: {
    // 获取个人信息
    async getPersonalDetail() {
      const data = await getPersonalDetail(this.userId) // 获取员工数据
      if (data.staffPhoto) {
        // 如果存在员工照片
        this.$refs.staffPhoto.fileList = [{ upload: true, url: data.staffPhoto }]
      }
      this.formData = data
    },
    // 基本信息的保存
    ...
    async getUserDetailById() {
      const data = await getUserDetailById(this.userId) // 获取用户资料
      // 这里判断如果存在图片 传入一个文件列表
      if (data.staffPhoto) {
        // 如果存在员工照片
        this.$refs.myStaffPhoto.fileList = [{ upload: true, url: data.staffPhoto }]
      }
      this.userInfo = data
    }
  }
